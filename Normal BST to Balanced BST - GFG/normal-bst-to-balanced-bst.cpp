...
class Solution{
    ...
    Node* solve(vector<int>&nodes,int s,int e){
        if(s>e)return NULL;
        int mid = s + (e-s)/2;
        Node* root = new Node(nodes[mid]);
        root ->left = solve(nodes,s,mid-1);
        root ->right = solve(nodes,mid+1,e);
        return root;
    }
    Node* buildBalancedTree(Node* root)
    {
    	...
    	//Create a balanced BST from scratch
    	vector<int>nodes;
    	inorder(root,nodes);
    	int n = nodes.size();
    	Node * ans = solve(nodes,0,n-1);
    	return ans;
